...
import { CustomReturnType, IListing, IListingPostBody, IListingResponseObject, LisitingType } from 'src/interfaces/listing.model';
...
export class ListingsService {
  ...
  private _type: LisitingType = "GetAllListings";
  private _apikey = this.getAPIKey() ?? '';
  private _columns = ['id', 'title', 'location', 'price', 'bedrooms', 'bathrooms', 'url', 'parking spaces', 'amenities', 'description', 'type', 'images'] as CustomReturnType
  private _listingPostBody : IListingPostBody = {apikey: this._apikey, type: this._type, return: this._columns }
  ...
  public setListingPostBody(_body?: IListingPostBody){
    this._listingPostBody = _body ? {...this._listingPostBody, ..._body} : this._listingPostBody
  }
  ...
  public get(): Observable<IListing[]> {
    ...
    let body = new HttpParams()
     ...
     .set('return', (_body && Array.isArray(_body?.return)) ? _body.return.join(',') : '');
